...
const FooterMenuChange=(props)=>{
    ...
    const FooterProjectOptions=()=>{
        ...
        {footerProjectsClient.map((footerProject,index)=>
                <div key={index} className="flex flex-col md:flex-row justify-evenly items-center gap-2 w-full">
                    <div className="flex flex-row items-center justify-center w-full">

                    <span>
                        Title:
                    </span>
                    <textarea name="hello" className="h-8 rounded-lg text-center flex-grow" defaultValue={footerProject['title']} onChange={(e)=>handleProjectsTitleChange(index,e.target.value)}></textarea>
                    </div>
                    <div className="flex flex-row items-center justify-center w-full">
                    ...
                    <textarea name="hello" className="h-8 rounded-lg text-center flex-grow" defaultValue={footerProject['href']} onChange={(e)=>handleProjectsLinkChange(index,e.target.value)}></textarea>
                    </div>
                </div>
            )}
    ...
    const FooterServiceOptions=()=>{
        return (
            ...
            {footerServicesClient.map((footerService,index)=>
                    <div key={index} className="flex flex-row justify-evenly items-center gap-3 w-full">
                        <div className="flex flex-row items-center justify-center w-full">
                        <span>
                            Title:
                        </span>
                        <textarea name="title" className="h-8 rounded-lg text-center flex-grow" defaultValue={footerService['title']} onChange={(e)=>handleServicesTitleChange(index,e.target.value)}></textarea>
                        </div>
                        <div className="flex flex-row items-center justify-center w-full">
                        <span>
                            Link:
                        </span>
                        <textarea name="href" className="h-8 rounded-lg text-center flex-grow" defaultValue={footerService['href']} onChange={(e)=>handleServicesLinkChange(index,e.target.value)}></textarea>
                        </div>
                    </div>
                )}
    ...
    return(
        ...
        <div className="relative w-full h-full backdrop-blur-lg backdrop-filter flex flex-col justify-center items-center">
          <div className="absolute w-10/12  bg-slate-200 rounded-lg flex flex-col">
