...
const MenuChange = (props) => {
  ...
  const deleteAMenuBox = (menuToDelete) => {
    const updatedMenus = menusClient.filter(menu => menu !== menuToDelete);
    console.log(updatedMenus);
    changeClientMenu(updatedMenus);
    console.log(menusClient);
  };
      ...
      <div className="relative w-full h-full backdrop-blur-lg backdrop-filter flex flex-col justify-center items-center">
        <div className="absolute w-10/12  bg-slate-200 rounded-lg flex flex-col">
          {menusClient.map((menu, index) => (
            <div key={index} className="flex flex-col md:flex-row justify-evenly items-center m-1 p-2 border-b-2 border-gray-300 gap-2 menuBoxes">
              ...
              <textarea name="hello" className="menu-href-header h-8 rounded-lg text-center" defaultValue={menu['href']}></textarea>
                ...
                <button className="px-6 py-2 bg-red-500 rounded-lg text-white" onClick={() => deleteAMenuBox(menu)}>Delete</button>
            </div>
          ))}
